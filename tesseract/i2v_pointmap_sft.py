...
from tesseract.modules.tesseract_pipeline import TesserActImageToPointmapVideoPipeline
from tesseract.modules.tesseract_model import TesserActPointmap as CogVideoXTransformer3DModel


from args import get_args  # isort:skip
from robodataset import RoboDepthNormal, crop_and_resize_frames, BucketSampler, RoboPointmap
...
def run_validation(
    ...
    pipe = TesserActImageToPointmapVideoPipeline.from_pretrained(
    ...
    def load_image_data(image_path: str) -> torch.Tensor:
        ...
        if "asset" in image_path:
            pointmap_path = image_path.replace(".png", "_depth.npy")
            pointmap_image = np.load(pointmap_path)
            pointmap_image = 1 - pointmap_image
        elif "mp4" in image_path:
            pointmap_path = image_path.replace("video/rgb.mp4", "pointmap/npz/pointmap.npz")
            pointmap_image = np.load(pointmap_path)["point_map"][0].astype(np.float32)
        else:
            pointmap_image = np.load(image_path.replace("rgb/0.png", "pointmap/npz/pointmap.npz"))["point_map"][0].astype(np.float32)

        pointmap_image = crop_and_resize_frames([pointmap_image], (H, W))[0]
        
        # Normalize to [0, 1]
        x_min, x_max = pointmap_image[..., 0].min(), pointmap_image[..., 0].max()
        y_min, y_max = pointmap_image[..., 1].min(), pointmap_image[..., 1].max()
        z_min, z_max = pointmap_image[..., 2].min(), pointmap_image[..., 2].max()
        center = np.array([
            (x_max + x_min) / 2,
            (y_max + y_min) / 2,
            (z_max + z_min) / 2
        ...
        pointmap_image = (pointmap_image - center) / scale + 0.5
        pointmap_image = torch.from_numpy(pointmap_image).to(dtype=weight_dtype, device=accelerator.device)

        # Combine all channels
        image = torch.cat([rgb_image, pointmap_image, pointmap_image], dim=-1)
...
def main(args):
    ...
    train_dataset = RoboPointmap(**dataset_init_kwargs)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                image_latents_rgb = image_latents[:, :, : C // 2].clone()
                image_latents_pointmap = image_latents[:, :, C // 2 : C // 2 * 2].clone()
                noisy_video_latents_rgb = noisy_video_latents[:, :, : C // 2].clone()
                noisy_video_latents_pointmap = noisy_video_latents[:, :, C // 2 : C // 2 * 2].clone()
                noisy_model_input = torch.cat(
                    [
                        noisy_video_latents_rgb,
                        image_latents_rgb,
                        noisy_video_latents_pointmap,
                        image_latents_pointmap,
                    ],
                    dim=2,
                )
                ...
                mask = batch["masks"].to(accelerator.device, non_blocking=True)  # [B, 2]
                C = target.shape[2]
                rgb_mask, pointmap_mask = mask[:, 0], mask[:, 1]
                target[pointmap_mask == 0, :, C // 2 : C // 2 * 2] = 0
                model_pred[pointmap_mask == 0, :, C // 2 : C // 2 * 2] = 0
                rgb_pred, pointmap_pred = model_pred.chunk(2, dim=2)
                rgb_target, pointmap_target = target.chunk(2, dim=2)
                ...
                pointmap_loss = loss_fn(pointmap_pred, pointmap_target, weights)
                loss = rgb_loss + pointmap_loss
            ...
            if accelerator.is_main_process:
                writer.add_scalars(
                    "Loss",
                    {
                        ...
                        "Pointmap": pointmap_loss.detach().item(),
                    },
                    global_step,
                )
